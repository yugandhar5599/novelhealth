...
<body>
	<div class="panel panel-success">
	  ...
	  <div class="panel-heading">Nurse Management Board</div>
	  <div class="col-md-6 col-sm-12 btn-margin-bottom action-buttons">
        <div class="btn-group" id="divAddNew">
            <a title="Add new" class="btn btn-block btn-danger btn-flat" href="./getNurseRegPage"><span class="hide-on-mobile">Add New </span><i class="fa fa-plus"></i></a>
        </div>
        ...
        <form class="navbar-form navbar-right" role="search" action="./searchNurses">
	  ...
	  <table class="table">
	  	...
	  	<c:if test="${uiAdminsList >= '0'}">
		  	<c:forEach items="${uiAdminsList}" var="nurseBean">
			  	<tr>
			  		<td>${nurseBean.nurseId}</td>
			  		<td>${nurseBean.hospitalName}</td>
			  		<td>${nurseBean.firstName}</td>
			  		<td>${nurseBean.lastName}</td>
			  		<td>${nurseBean.email}</td>
			  		<td>${nurseBean.phone}</td>
			  		<td>${nurseBean.status}</td>
			  		<td><a href="./editNurse?nurseId=${nurseBean.nurseId}"><div class="glyphicon glyphicon-edit"></div></a></td>
			  		<td><a href="./deletenurse?nurseId=${nurseBean.nurseId}"><div class="glyphicon glyphicon-trash"></div></a></td>
			  	</tr>
		  	</c:forEach>
	  	</c:if>
	  </table>
	...
	<nav>
	  <ul class="pagination">
	  	<li>
	      <a href="./getAllNursesByPagening?currentPage=${loop.count-1}" aria-label="Previous">
	        <span aria-hidden="true">&laquo;</span>
	      </a>
	    </li>
	    
	  	<c:forEach items="${pageBarList}" varStatus="loop" var="pageBarNumber">
	  		<li><a href="./getAllNursesByPagening?currentPage=${loop.count}"><span>${loop.count}<span class="sr-only">(current)</span></span></a></li>
	  	</c:forEach>
	    
	    
	    <li>
	      <a href="./getAllNursesByPagening?currentPage=${loop.count+1}" aria-label="Next">
